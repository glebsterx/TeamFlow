...
- [x] #355 — Тесты: CardsView (карточка задачи) — **DONE** 22.09.2026 (4 теста:
  клик по карточке открывает задачу, чекбокс bulk-select не открывает задачу,
  кнопка "Взять"/"Начать" в зависимости от assignee). `TaskCard`/`Modal` из
  исходной формулировки не существуют — компонент карточки называется
  `CardsView` (см. FIX-27), `TaskModal` — отдельная крупная форма, не покрыта.
- [x] #356 — Тесты: login form — **DONE** 22.09.2026 (5 тестов: рендер формы,
  нормализация логина + сохранение токенов + редирект, показ ошибки сервера,
  валидация регистрации — несовпадение паролей и короткий пароль).
- [x] #357 — Тесты: Dashboard — **DONE** 22.09.2026 (2 теста: смоук-рендер шелла
  с реальными query-хуками на моках axios, переключатель вида задач пишет
  выбор в localStorage). Полный интеграционный охват Dashboard.tsx (1055
  строк, ~10 useQuery, десяток под-страниц) не покрыт — см. PLAN.md, это
  сознательно сокращённый скоуп.
...
- [x] **#333** — **Рефакторинг: отделить Auth System от Task System** (10 подзадач #334-343) — **закрыто 22.09.2026 (решение: удалить дубли, не доводить рефакторинг)**
  - Новые `repositories/auth|tasks/`, `services/auth|tasks/` были созданы, но никем не
    ...
    (в новом `task_service` жил NameError-баг). Исходники удалены 23.08.2026 (см. PLAN.md
    FIX-13) — быстрее и безопаснее, чем доводить рефакторинг до конца. Чекбокс не был
    проставлен тогда; проверено 22.09.2026 — нет ни одного call site на
    `repositories.tasks/auth` или `services.tasks/auth` во всём backend, `core/user_context.py`
    по-прежнему не существует. От той попытки остались только пустые (root-owned,
    негитованные) `__pycache__`-директории на диске — не код, чистить не критично.
