...
### Backend/Frontend
- Найден и починен реальный баг первого запуска: шаг 3 SetupWizard (сохранение
  системных настроек, `PUT /settings/system`) не отправлял `Authorization`,
  хотя эндпоинт требует JWT — мастер настройки не мог завершиться ни для
  одного нового инстанса без ручного skip
- E2E-инфраструктура (Playwright): изолированный стек
  `docker-compose.e2e.yml` (свой `name:`, чтобы не коллизировать с прод —
  см. инцидент в комментарии файла), `scripts/e2e.sh` (build → up → test →
  teardown), `frontend/e2e/core-flow.spec.ts` — проходит первый запуск
  (Setup Wizard → создание задачи → смена статуса), проверяет Welcome-страницу
