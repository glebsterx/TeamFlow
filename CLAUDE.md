...
- **Прод:** LXC `apps-home-01` (192.168.0.11, `ssh apps`, код в `/data/TeamFlow`)
- **Тест/dev:** LXC `apps-dev-home-01` (192.168.0.18, `ssh apps-dev`, VMID 207 на pve-home-02)

Q190 (`srv-home-01`) остаётся местом, где правится код и запускается dev-стек (решение 2026-08-18); `apps-dev-home-01` — назначенный тест-сервер, выкладка вручную (`rsync` + `docker compose up -d --build`). Деплой прода — `rsync` на `apps-home-01`, не `git pull` (см. выше).
