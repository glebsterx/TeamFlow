...
- [x] **FIX-30 (S) — Свести на один внешний порт (нужно для homelab-каттовера на 80/443).** Закрыто 22.09.2026. `VITE_API_URL` на проде уже был пуст (фикс попал в `deploy.sh` раньше, вместе с FIX-28) — фронтенд ходит на бэкенд относительным путём через vite-прокси. `docker-compose.yml` на `apps-home-01` трогать было нельзя: `Caddy` на `edge-home-01` держал отдельный сайт-блок `https://tf.glebsterx.ru:8181 { forward_auth … ; reverse_proxy /* http://192.168.0.11:8182 }`, ходящий на бэкенд-порт напрямую по LAN, в обход фронтенда/vite-прокси (была предпринята попытка перевести `ports:` на `127.0.0.1:${BACKEND_PORT}:8000` — тут же сломала `edge → backend`, откачено в тот же день, см. git history).
  Фикс сделан на стороне `homelab`: блок `tf.glebsterx.ru:8181` в Caddyfile на `edge-home-01` удалён — он оказался мёртвым: WAN-проброс `8180→8181` на роутере на самом деле ведёт на `gls.glebsterx.ru`, не на TeamFlow (см. `external-exposure.md` в `homelab`); OAuth `redirect_uri` в БД и так указывает на несуществующий порт `8180`; Telegram-бот — polling, не webhook. `gls.glebsterx.ru:8181` (делил порт по SNI) не затронут, проверено вживую. TeamFlow снаружи теперь только на `:5181`; `docker-compose.yml` в этом репозитории не менялся — конечный fix целиком в `homelab` (`docs/edge-full-cutover-80-443-plan.md`, коммит `55c4f23`). Голый 443 без порта — отдельное решение, вне объёма этого пункта.
