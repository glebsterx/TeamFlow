...
- [x] **FIX-30 (S) — Свести на один внешний порт (нужно для homelab-каттовера на 80/443).** Закрыто 22.09.2026. `VITE_API_URL` на проде оказался уже пустым (фикс попал в `deploy.sh` раньше, вместе с FIX-28) — фронтенд и так ходит на бэкенд относительным путём через vite-прокси. Оставалось убрать лишнюю внешнюю публикацию бэкенд-порта: `docker-compose.yml` — `ports:` бэкенда переведены на `127.0.0.1:${BACKEND_PORT}:8000` (было `0.0.0.0`), т.к. фронтенд обращается к бэкенду по docker-network (`backend:8000`), а не через published-порт. Применено на проде (`apps-home-01`): `docker compose up -d --force-recreate backend`. Проверено — `curl http://localhost:5180/health` → 200, `/api/tasks` → 401 (не 502/504) через прокси фронтенда. Наружу теперь только порт фронтенда (5180); отключение внешнего проброса backend-порта (8182) на роутере/файрволе — homelab-каттовер, вне этого репозитория, см. `docs/edge-full-cutover-80-443-plan.md` в репозитории `homelab`.
