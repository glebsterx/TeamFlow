...
- [ ] **FIX-30 (S) — Свести на один внешний порт (нужно для homelab-каттовера на 80/443).** Находка подтверждена 22.09.2026: `VITE_API_URL` на проде уже пуст (фикс попал в `deploy.sh` раньше, вместе с FIX-28) — фронтенд ходит на бэкенд относительным путём через vite-прокси, а не по абсолютному URL с портом.
  **Важно (проверено на edge-home-01 22.09.2026):** несмотря на это, отвязывать бэкенд-порт от `0.0.0.0` на `apps-home-01` **нельзя** — `Caddy` на `edge-home-01` держит отдельный сайт-блок `https://tf.glebsterx.ru:8181 { forward_auth … ; reverse_proxy /* http://192.168.0.11:8182 }`, который ходит на бэкенд-порт напрямую по LAN, а не через фронтенд/vite-прокси. Была предпринята попытка перевести `docker-compose.yml` на `127.0.0.1:${BACKEND_PORT}:8000` — это тут же сломало `tf.glebsterx.ru:8181` (edge → backend вернул connection refused), откачено в тот же день. Домен и так уже отдаётся по HTTPS с обоих портов (реальный ACME-сертификат через Caddy, см. `docs/edge-full-cutover-80-443-plan.md` и `docs/runbook-edge-caddy-authelia.md` в репозитории `homelab`) — открытый вопрос только про **число портов в URL**, а не про HTTPS.
  Свести к одному порту снаружи можно, только убрав/переписав сайт-блок `tf.glebsterx.ru:8181` в Caddyfile на `edge-home-01` (не трогая published-порт на `apps-home-01`) — то есть правкой в репозитории `homelab`, не здесь. До этого пункт остаётся открытым.
